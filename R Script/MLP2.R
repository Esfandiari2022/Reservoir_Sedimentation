...
model %>%
  layer_dense(units = 75, kernel_initializer = 'uniform', activation = "relu", name = "hidden_1", input_shape = c(3)) %>%
  layer_dense(units = 75, kernel_initializer = 'uniform', activation = "sigmoid", name = "hidden_2") %>%
...
# Normalizing
norm_data <- matrix(nrow =134356 , ncol =  3)
...
pred_data36 <- pred_data36[!duplicated(pred_data15$X), ]
pred_data36$Year <- 2036

# Normalizing
norm_data36 <- matrix(nrow =134356 , ncol =  3)
